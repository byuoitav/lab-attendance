...
function Deps {
    ...
    Set-Location "autoclave"
    Invoke-Expression "npm config set unsafe-perm true"
    Invoke-Expression "npm install --legacy-peer-dep"
    Invoke-Expression "cd .."
    Write-Output "Exiting \autoclave"
}
...
function Build {
    ...
    if (Test-Path "autoclave") {
        Set-Location "autoclave"
        Write-Output "Entering \autoclave"
        New-Item -Path dist -ItemType Directory
        #Invoke-Expression "npm run-script build"
        Invoke-Expression "npm run ng build" # --aot --optimization --base-href='/autoclave/'"
        Invoke-Expression "cd .."
        Write-Output "Exiting \autoclave and moving files to \dist"
        Move-Item "$location\autoclave\dist\autoclave" -Destination "$location\dist\autoclave-dist"

        if (Test-Path -Path "autoclave\dist") {
            Remove-Item autoclave\dist -recurse
            Write-Output "Recursively deleted autoclave\dist/"
        } else {
            Write-Output "No autoclave\dist directory to delete"
        }
